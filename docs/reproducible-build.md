...
2026-08-27 后续维护：构建器已锁定至 26.15.3，修复此前 12 项构建依赖审计告警，并适配运行时资源复制规则。当前的审计、18 项不同检查、独立输出、回滚步骤与证据见[工具链维护记录](toolchain-hardening-2026-08-27.md)。已验收的 0.1.2 产物未覆盖；下文的旧审计数字保留为历史证据，不代表当前工具链状态。

同日后续已完成[公开源码备份](publication-2026-08-27.md)：桌面仓库及完整 Harness 补丁已上传到 `yangshulin2333/dsh-desktop`，并从公开 URL 拉回核对。独立 Harness fork 尚未创建；下文的源码恢复方法仍适用。
...
本页原构建验证轮次没有创建远端仓库或修改 upstream remote；本地提交当时不等于远端备份。后续桌面源码、历史及 Harness 恢复补丁已公开上传并核验，见[发布记录](publication-2026-08-27.md)。仍不要向官方 upstream 推送本地 Harness 功能分支；若需要独立 fork，再单独安排。
