...
日期：2026-08-27。状态：代码修复、两个包的生成、17 项自动检查通过；**用户回复“没问题了”，所报任务栏图标/名称问题验收通过并关闭**。安装包及便携版入口仍未单独验收。没有操作用户桌面、启动 GUI、改 Windows 开发者模式、清图标缓存或重启 Explorer。
...
2026-08-27，提供上述 0.1.2 解包版路径及取消旧固定、退出旧版、重新固定、关闭后从任务栏启动的检查步骤后，用户回复“没问题了”。据此记录本次所报的固定后图标回退及右键应用入口问题已获用户确认，关闭该 bug。

证据为用户对修复及检查步骤的整体确认，没有新增截图或逐项操作日志，代理未独立观察固定/重启过程。本轮只读检查未发现运行中的 `DSH Desktop.exe` 或匹配的固定快捷方式，因此不声称本轮另行核实了实际运行路径。该限制不撤销用户反馈，也不把结论扩展为安装包、便携启动器或全新机器的完整验收。

本轮仅更新验收与交接状态，没有修改程序、重新打包或重跑此前已通过的后台测试；原产物及哈希保持不变。
